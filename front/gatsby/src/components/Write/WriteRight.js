    ...
    <nav className={`${expanded ? styles.expandRight : styles.retractRight}`}>
      ...
      {expanded && <>
        <div className={styles.yamlEditor}>
          ...
          {selector === "raw" && <>{error !== "" && <p className={styles.error}>{error}</p>}<textarea value={rawYaml} onChange={(event) => {
          ...
          {selector !== "raw" && props.readOnly && <YamlEditor yaml={props.yaml} basicMode={selector === 'basic'} error={(reason) => {
            setError(reason);
            if (reason !== "") {
              setSelector("raw")
            }
