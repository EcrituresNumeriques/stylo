...
export default ({ yaml }) => {
  const [parsed] = YAML.safeLoadAll(yaml, { schema: SEXY_SCHEMA })
...
export default class YamlEditor extends Component {
  ...
  componentWillReceiveProps(nextProp) {
    const singleYaml = nextProp.yaml
      .replace(/[-]{3}\n/g, '')
      .replace(/\n[-]{3}/g, '')
  ...
  cleanOutput(object) {
    ...
    for (var propName in cleaning) {
      ...
      if (
        Array.isArray(cleaning[propName]) &&
        cleaning[propName].length === 0
      ) {
        delete cleaning[propName]
      }
    }
    ...
    //Update only the key changed, plus export the new state
      ...
      if (typeof removeFromArray == 'number') {
        this.setState((state) => {
          ...
          _.set(state, 'obj.' + target, [
            ...nextArray.slice(0, removeFromArray),
            ...nextArray.slice(removeFromArray + 1),
          ])
          return state
        })
      } else {
    ...
    if (type === 'rubriques') {
      this.setState((state) =>
        _.set(
          state,
          'obj.typeArticle',
          state.misc.rubriques.filter((r) => r.selected).map((r) => r.label)
        )
      )
      ...
      this.setState(function (state) {
        ...
        if (toSet.length > 0) {
          ...
          state.obj.controlledKeywords = state.misc.categories
            .filter((c) => c.selected)
