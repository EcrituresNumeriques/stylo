...
export default function ArticleEditorMetadataForm({ metadata, onChange = () => {} }) {
  const formData = convertLegacyValues(metadata)
  const schemaMerged = useMemo(() => merge(schema), [schema])
  const handleChange = useCallback((newFormData) => onChange(newFormData), [onChange])
  return <Form formData={formData} schema={schemaMerged} uiSchema={uiSchema} onChange={handleChange}/>
}
