...
export default function CollaborativeSessionError ({ error }) {
  ...
  if (error === 'notFound') {
    return <ErrorMessageCard title="Important">
      <Text>This collaborative session <Code>{collaborativeSessionId}</Code> has ended.</Text>
      <Text>Please start a new one.</Text>
    </ErrorMessageCard>
  }
  return <ErrorMessageCard title="Error">
