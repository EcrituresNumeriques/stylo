...
export default function CollaborativeEditorWriters () {
  ...
  return (<Avatar.Group>
    {Object.entries(articleWriters).map(([key, writer]) => {
      return <Tooltip key={key} text={writer.user?.displayName} className={styles.avatar} placement="topStart" leaveDelay={0}>
        <Avatar text={writer.user?.displayName} stacked/>
        <div className={styles.dot} style={{ backgroundColor: writer.user?.color }}/>
      </Tooltip>
    })}
  </Avatar.Group>)
}
