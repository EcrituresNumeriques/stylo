...
export default function ArticleContributorAdd ({ article, cancel }) {
  ...
  const allContributors = useMemo(() => merge(contributors, contacts), [contacts, contributors])
  ...
  useEffect(() => {
    runQuery({ query: queries.getAcquintances, variables: { user: userId } })
      .then(({ user }) => {
        setLoading(false)
        setContacts(user.acquintances)
      })
  }, [])

  const refreshContacts = useCallback((acquintances) => setContacts(acquintances), [])
