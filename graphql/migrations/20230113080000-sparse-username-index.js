exports.up = async function (db) {
  ...
  try {
    ...
    const duplicateUsernameList = await users
      .aggregate([
        {
          $match: {
            username: {
    ...
    for (const item of duplicateUsernameList) {
      for (const duplicate of item.duplicates) {
        console.log(
          `Username ${item.name} is non unique, update username to email on user with id: ${duplicate}`
        )
        await users.updateOne({ _id: duplicate }, [
          {
            $set: {
              username: '$email',
            },
